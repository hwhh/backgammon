...
if __name__ == '__main__':
    ...
    pieces.extend([Piece(loc, 'b') for loc in zip([12] * 5, range(5))][::-1])

    pieces.extend([Piece(loc, 'b') for loc in zip([4] * 3, range(3))])
    pieces.extend([Piece(loc, 'w') for loc in zip([16] * 3, range(3))][::-1])

    pieces.extend([Piece(loc, 'b') for loc in zip([6] * 5, range(5))])
    pieces.extend([Piece(loc, 'w') for loc in zip([18] * 5, range(5))][::-1])

    pieces.extend([Piece(loc, 'w') for loc in zip([11] * 2, range(2), )])
    pieces.extend([Piece(loc, 'b') for loc in zip([23] * 2, range(2))][::-1])
