...
class Board:
    ...
    def validate_move(self, piece, dest, combo):
        if 0 <= dest <= 23 and (len(self.pieces[dest]) <= 1 or (self.pieces[dest][-1]).colour == piece.colour):
            return True
        if combo and self.can_bear_off_piece(piece, dest):
            return True
        return False
